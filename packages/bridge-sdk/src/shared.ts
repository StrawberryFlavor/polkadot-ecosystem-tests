...
import { check } from '../../shared/src/helpers'
import { setupNetworks } from '@e2e-test/shared'

export const buildTests = (tests) => {
  for (const { from, to, token } of tests) {
    describe(`'${from}' to '${to}' using bridgeSDK cross-chain '${token}'`, async () => {
      async function chooseChain(chain: string) {
        const network = {
          karura: karura,
          kusama: kusama,
          moonriver: moonriver,
      ...
      beforeEach(async () => {
        const [fromchain1, tochain1] = await setupNetworks(await chooseChain(from), chooseChain(to))
